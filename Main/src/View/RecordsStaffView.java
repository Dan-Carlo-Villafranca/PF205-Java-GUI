...
public class RecordsStaffView extends javax.swing.JFrame {

    public RecordsStaffView() {
        initComponents();
        acc_name1.setText(config.Session.name);
        acc_type2.setText(config.Session.type);
        getStocksData();
        // Set the dropdown to 'Full Inventory' by default and load it
        reportTypeSelect.setSelectedIndex(0); 
        loadReport("SELECT b_id AS 'ID', blood_type AS 'Type', quantity AS 'Units' FROM tbl_blood");
    }
    ...
    try (java.sql.Connection conn = con.connectDB();
        ...
        // This helper line (from the rs2xml library or manual loop) 
        // fills your JTable with the database results instantly.
    ...
    private void initComponents() {
        ...
        reportTypeSelect = new javax.swing.JComboBox<>();
        ...
        reportTypeSelect.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Full Inventory", "Critical Count", "Donation History", "Request History" }));
        reportTypeSelect.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                reportTypeSelectActionPerformed(evt);
            }
        });
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addComponent(reportTypeSelect, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
    ...
    private void reportTypeSelectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_reportTypeSelectActionPerformed
        String selected = reportTypeSelect.getSelectedItem().toString();

            if (selected.equals("Full Inventory")) {
                // Simple inventory view
                loadReport("SELECT b_id AS 'ID', blood_type AS 'Type', quantity AS 'Units' FROM tbl_blood");
            } 
            else if (selected.equals("Critical Count")) {
                // Only shows low stock
                loadReport("SELECT b_id AS 'ID', blood_type AS 'Type', quantity AS 'Units' FROM tbl_blood WHERE quantity < 5");
            } 
            else if (selected.equals("Donation History")) {
                // JOIN to get Donor Name + Formatted Date
                String sql = "SELECT d.d_id AS 'ID', a.full_name AS 'Donor Name', " +
                             "d.blood_type AS 'Type', " +
                             "strftime('%Y-%m-%d', d.d_date / 1000, 'unixepoch') AS 'Date' " +
                             "FROM tbl_donations d " +
                             "JOIN tbl_accounts a ON d.u_id = a.u_id";
                loadReport(sql);
            } 
            else if (selected.equals("Request History")) {
                // JOIN to get Recipient Name + Formatted Date
                // Note: We use a.full_name from tbl_accounts instead of a status
                String sql = "SELECT r.r_id AS 'ID', a.full_name AS 'Recipient', " +
                             "r.blood_type AS 'Type', " +
                             "strftime('%Y-%m-%d', r.r_date / 1000, 'unixepoch') AS 'Date' " +
                             "FROM tbl_requests r " +
                             "JOIN tbl_accounts a ON r.u_id = a.u_id";
                loadReport(sql);
            }
    }//GEN-LAST:event_reportTypeSelectActionPerformed
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new RecordsStaffView().setVisible(true);
            }
            }
        });
    }
    ...
    public javax.swing.JComboBox<String> reportTypeSelect;
